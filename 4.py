...
def Df2 (x):
    ...
    #Df2x = x[1]
    #Df2y = x[0]
    return np.array([[2*x[0], -1*x[1]], [x[1], x[0]]])

def f3(x):
    #fx1 = x[0]/2 * sin(np.pi * x[0] / 2) - x[1]
    #fx2 = x[1] ** 2 - x[0] + 1
    return np.array([x[0]/2 * sin(np.pi * x[0] / 2) - x[1], x[1] ** 2 - x[0] + 1])

def Df3 (x):
    #Df1x = np.sin(np.pi * x[0] / 2) / 2 + x[0] * np.pi / 4 * np.cos(np.pi * x / 2)
    #Df1y = -1
    #Df2x = 1
    #Df2y = 2 * x[1]
    return np.array([[np.sin(np.pi * x[0] / 2) / 2 + x[0] * np.pi / 4 * np.cos(np.pi * x / 2), -1], [[1], 2 * x[1]]])
...
def main():
    ...
    for i in range (0, num_points):
        x5[i], it5[i] = newton_C(x5_0[i], j, f5, Df5, 0.0001, it5[i])
        print (x5[i])
